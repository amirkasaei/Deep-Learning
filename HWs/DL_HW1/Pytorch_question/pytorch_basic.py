...
def multiples_of_ten(start, stop):
  ...
  if start > stop:
    ...
    x = torch.arange(start, stop+1, step=10).double()
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return x
...
def reverse_rows(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  indicies = torch.arange(-1, -(x.shape[0]+1),-1)
  y = x[indicies, :]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def take_one_elem_per_col(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  row_index = torch.tensor([1, 0, 3])
  col_index = torch.tensor([0, 1, 2])
...
def reshape_practice(x):
  ...
  # Permute dimensions to get the desired order
  y_permuted = x_reshaped.permute(1, 0, 2)
  
  # Reshape to get the final shape (3, 8)
  y = y_permuted.reshape(3, 8)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def zero_row_min(x):
  ...
  # Create a mask to identify the minimum values
  min_indicies = x == row_min_values
   
  # Replace the minimum values with 0
  y = x.clone()  # Create a copy of x
  y[min_indicies] = 0
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  if use_loop:
    # Initialize an empty list to store results for each batch
        ...
        # Perform batched matrix multiplication with explicit loop
        for i in range(x.shape[0]):
            z_batch = torch.mm(x[i], y[i])
...
def normalize_columns(x):
  ...
  var = torch.sum((x - mu)**2, dim=0) / (x.shape[0] - 1)
